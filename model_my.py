...
class J0(Base):
    def __init__(self, num_features=128):
        ...
        self.refine = nn.Sequential(
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1), nn.SELU(),  # 新增的卷积层
            nn.Conv2d(num_features, num_features, kernel_size=1)
        )
...
class DM2FNet(Base):
    ...
    def forward(self, x0, x0_hd=None):
        ...
        log_log_x = torch.log(torch.log(x0.clamp(min=1e-8) + 1) + 1)
        log_inverse_x0 = torch.log(1 / x0.clamp(min=1e-8, max=(1 - 1e-8)) + 1)
        log_sqrt_x0 = torch.log(torch.sqrt(x0.clamp(min=1e-8) + 1))
        ...
        x_j1 = torch.exp(x0 + r1).clamp(min=0., max=1.)
        ...
        #x_j3 = torch.log(torch.exp(x0 + r3)).clamp(min=0., max=1.)
        
        x_j3 = torch.exp(log_inverse_x0 + r3).clamp(min=0., max=1.)
        ...
        x_j4 = torch.exp(log_sqrt_x0 + r4).clamp(min=0., max=1.)
